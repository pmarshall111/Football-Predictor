function [all_theta] = oneVsAllWithProbabilitiesScoreOdds(X, y, num_labels, lambda, probabilityOfResults, scores, bookiesOdds)
...
highestBookiesOdds = sum(bookiesOdds,2);

# Logistic Regression for a home win
initial_theta = zeros(n + 1, 1);
options = optimset('GradObj', 'on', 'MaxIter', 500);
[theta] = fmincg (@(t)(lrCostFunctionWithProbabilitiesScoreOdds(t, X, (y == 1), abs(goalsAwayFromHomeWin), probabilityOfResults, lambda, highestBookiesOdds)), initial_theta, options);      
all_theta(1,:) = theta;

# Logistic Regression for a draw
initial_theta = zeros(n + 1, 1);
options = optimset('GradObj', 'on', 'MaxIter', 500);
[theta] = fmincg (@(t)(lrCostFunctionWithProbabilitiesScoreOdds(t, X, (y == 2), abs(goalsAwayFromDraw), probabilityOfResults, lambda, highestBookiesOdds)), initial_theta, options);      
all_theta(2,:) = theta;

# Logistic Regression for an away win
initial_theta = zeros(n + 1, 1);
options = optimset('GradObj', 'on', 'MaxIter', 500);
[theta] = fmincg (@(t)(lrCostFunctionWithProbabilitiesScoreOdds(t, X, (y == 3), abs(goalsAwayFromAwayWin), probabilityOfResults, lambda, highestBookiesOdds)), initial_theta, options);      
all_theta(3,:) = theta;
