...
lambda = 16; %hard coded lambda. We don't want function as that hides all the variables we create within it's scope.
fprintf("Training for lambda value of %f. Reading in data.\n", lambda)

trainingSet = csvread(strcat(fileparts(mfilename('fullpath')), "/../data/01Feb22/train.csv"));
#trainingSet = csvread(strcat(fileparts(mfilename('fullpath')), "/../data/01Feb22/nolineups_train.csv"));
X = trainingSet(:, 9:end);
...
testSet = csvread(strcat(fileparts(mfilename('fullpath')), "/../data/01Feb22/test.csv"));
#testSet = csvread(strcat(fileparts(mfilename('fullpath')), "/../data/01Feb22/nolineups_test.csv"));
testX = testSet(:, 9:end);
